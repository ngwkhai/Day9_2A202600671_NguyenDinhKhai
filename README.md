...
| LLM provider | OpenAI API by default, OpenRouter fallback |
...
│   ├── llm.py                 # get_llm() → ChatOpenAI via OpenAI/OpenRouter fallback
...
| `OPENROUTER_API_KEY` | Optional OpenRouter fallback key | unset |
| `OPENROUTER_MODEL` | Optional OpenRouter model identifier | `anthropic/claude-sonnet-4-5` |
| `REGISTRY_URL` | Registry service URL | `http://localhost:10000` |

The model is swappable via `OPENAI_MODEL` for OpenAI, or via `OPENROUTER_MODEL` when using the fallback.
