...
def search_privacy_law(query: str) -> str:
    """Search privacy and data-protection knowledge base for applicable obligations.

    Args:
        query: Natural language query about privacy law, GDPR, or data breaches.
    """
    knowledge = [
        (
            ["data", "breach", "leak", "rò", "dữ liệu"],
            "Data breaches may trigger breach-notification duties, regulator reporting, "
            "customer notice, forensic investigation, remediation, and potential class-action "
            "claims. GDPR notification to a supervisory authority is generally required within "
            "72 hours when a breach risks individual rights and freedoms.",
        ),
        (
            ["privacy", "gdpr", "consent", "personal"],
            "GDPR requires a lawful basis for processing personal data, transparency notices, "
            "data-subject rights, data minimisation, and security controls. Serious violations "
            "can lead to fines up to EUR 20M or 4% of annual global turnover.",
        ),
        (
            ["ccpa", "cpra", "california", "consumer"],
            "CCPA/CPRA grants California consumers rights to know, delete, correct, and opt out "
            "of sale or sharing of personal information. Statutory damages for certain breaches "
            "can range from $100 to $750 per affected consumer per incident.",
        ),
    ]
    query_lower = query.lower()
    results = []
...
async def check_routing(state: LegalState) -> dict:
    ...
    messages = [
        SystemMessage(
            content=(
                ...
                'needs_privacy = true → question involves data privacy, GDPR, consent, or data breaches'
            )
        ),
        HumanMessage(content=state["question"]),
    ]
    ...
    needs_tax = bool(parsed.get("needs_tax", True)) or any(
        kw in question_lower for kw in ["tax", "irs", "thuế", "revenue"]
    )
    needs_compliance = bool(parsed.get("needs_compliance", True)) or any(
    ...
    needs_privacy = bool(parsed.get("needs_privacy", False)) or any(
        kw in question_lower for kw in ["data", "privacy", "gdpr", "dữ liệu", "breach", "leak"]
    )
    print(
        "  [Node: check_routing] "
...
async def call_privacy_specialist(state: LegalState) -> dict:
    ...
    privacy_prompt = (
        "You are a senior privacy counsel specialising in GDPR, CCPA/CPRA, data-breach "
        "response, consent management, data-subject rights, and cross-border transfer risks. "
